...
class Receiver(Server):
    def __init__(
        ...
        self.routes["/ezrpc", "POST"] = RouteHandler("POST", )

    def function(self, name: str = None, await_result: bool = False, description: str = None) -> Callable:
    ...
    async def discover(self) -> ReceiverResponse:
        result = {func_name: handler.discover() for func_name, handler in self.functions}
